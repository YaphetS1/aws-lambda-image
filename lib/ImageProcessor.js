...
class ImageProcessor {
    ...
    processImage(imageData, config) {
        ...
        gm(imageData.data).size((err, value) => {
            if (err) {
                console.log(err)
            }
            else {
                console.log(value);
                let post_data = JSON.stringify(value);

                return new Promise((resolve, reject) => {
                    const options = {
                        host: 'api.dev.danch2night.com',
                        path: '/api/admin/photo/' + imageData.fileName.slice(imageData.fileName.lastIndexOf("/") + 1),
                        port: 443,
                        method: 'POST',
                        headers: {
